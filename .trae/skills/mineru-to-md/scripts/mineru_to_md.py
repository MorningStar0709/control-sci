...
STATS_DETAILS = re.compile(r"<details>", re.IGNORECASE)
STATS_MD_IMAGE = re.compile(r"!\[\]\(")
STATS_PIPE_TABLE = re.compile(r"^\|.+\|.+\|$", re.MULTILINE)
STATS_SCANNED_THRESHOLD_FORMULAS = 10
STATS_SCANNED_THRESHOLD_IMAGES = 50
...
def extract_stats(markdown: str) -> dict[str, Any]:
    block_matches = STATS_BLOCK_FORMULA.findall(markdown)
    block_count = len(block_matches)
    inline_count = len(STATS_INLINE_FORMULA.findall(markdown))
    details_count = len(STATS_DETAILS.findall(markdown))
    md_img_count = len(STATS_MD_IMAGE.findall(markdown))
    table_count = len(STATS_PIPE_TABLE.findall(markdown))

    is_scanned = (
        block_count < STATS_SCANNED_THRESHOLD_FORMULAS
        and (details_count + md_img_count) > STATS_SCANNED_THRESHOLD_IMAGES
    )

    return {
        "block_formulas": block_count,
        "inline_formulas": inline_count,
        "details_images": details_count,
        "md_images": md_img_count,
        "tables": table_count,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stats",
        action="store_true",
        help="读已保存的 Markdown 并提取公式/图片/表格计数，写入 saved 条目中的 stats 字段",
    )
...
def convert_one_file(
    ...
    if args.stats:
        try:
            md_content = Path(exported["output"]).read_text(encoding="utf-8")
            saved_item["stats"] = extract_stats(md_content)
        except Exception as stats_exc:  # noqa: BLE001
            eprint(f"警告: 统计信息提取失败: {stats_exc}")
            saved_item["stats"] = None
