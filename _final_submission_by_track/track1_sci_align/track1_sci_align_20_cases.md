...
| **Related capability** | A 维（概念回溯）— 准确回忆算法定义与数学表达式 |
...
| **Related capability** | A 维（概念回溯）— 基础定义与前提条件完整性 |
...
| **Related capability** | A 维（概念回溯）— 定义精确性，含权重系数的细节掌握 |
...
| **Related capability** | A 维（概念回溯）— 鲁棒控制基础概念精确性 |
...
| **Related capability** | B 维（多步推理）— 7 步分块矩阵运算的逐步推导 |
...
| **Related capability** | C 维（条件敏感性）— 参数估计误差对设计结论的影响 |
...
| **Related capability** | D 维（开放设计）— 参数规划 → 显式 MPC 的完整设计链 |
...
| **Related capability** | A 维（概念回溯）— 定理条件完整性 |
...
| **Related capability** | B 维（多步推理）— HJB 方程代入 → 化简 → 参数辨识 → 结论推导 |
...
| **Related capability** | C 维（条件敏感性）— 参数变化下平衡点偏移与稳定性条件的满足性检验 |
...
| **Related capability** | C 维（条件敏感性）— 环境参数变化对模型预测控制性能的影响分析 |
...
| **Related capability** | C 维（条件敏感性）— 参数变化不改变条件结构但改变可解性 |
...
| **Related capability** | D 维（开放设计）— CPS 混合自动机完整控制方案设计 |
...
| **Related capability** | A 维（概念回溯）— 多人博弈与均值场均衡定义 |
...
| **Related capability** | B 维（多步推理）— 复杂不等式拆解 → 系数计算 → 放缩估计 → 数值汇总 |
...
| **Related capability** | A 维（概念回溯）— Nyquist 稳定判据的数学基础 |
...
| **Related capability** | D 维（开放设计）— 经典控制根轨迹法全过程设计 |
...
| **Related capability** | C 维（条件敏感性）— 多项式矩阵秩的条件判断 |
...
| **Related capability** | D 维（开放设计）— 自适应控制完整方案 |
...
| **Related capability** | D 维（开放设计）— MPC 控制器全流程设计 |
...
| **Related capability** | A 维（概念回溯）— 滑模控制基本定义 |
...
| **Related capability** | A 维（概念回溯）— 事件触发机制核心条件 |
...
| **Related capability** | A 维（概念回溯）— 稳态误差与系统型别的关系 |
...
| **Related capability** | A 维（概念回溯）— 采样保持的数学模型与 LTV 性质 |
...
| **Related capability** | A 维（概念回溯）— 超前补偿器的参数体系 |
...
| **Related capability** | A 维（概念回溯）— 状态反馈与可控性的关系定理 |
...
| **Related capability** | B 维（多步推理）— 从阶跃响应数据反推系统参数 |
...
| **Related capability** | A 维（概念回溯）— 性能指标定义与特征 |
...
| **Related capability** | C 维（条件敏感性）— 非线性项符号变化对稳定性的根本影响 |
...
| **Related capability** | C 维（条件敏感性）— 测量条件对参数估计精度的根本性影响 |
