...
**统一收束**。本报告给出的不是孤立排行榜，而是可信科学文档智能体的度量基座：MinerU 负责把科学 PDF 变成可追溯结构化数据，四维 Benchmark 负责把模型能力差异量化，本地 / API 双路径负责把公开演示与私有部署分开。由此，数据集、Agent 与医疗 RAG 共同构成“数据基座→执行协议→行业落地”的同一条工程路线，而不是彼此独立的功能演示；控制科学作为基础工程学科，既提供了最严苛的数学结构测试场，也通过医学迁移验证了其方法论外延。
