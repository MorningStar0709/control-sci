...
本报告的开放交付重点不是发布一个医疗聊天页面，而是交付一套可被医院或科研机构替换本地资料后复用的证据服务骨架：MinerU 解析入口、医学 chunk 与索引构建脚本、REST API、Docker Compose、CLI 复核命令、中文 Ask trace、20 cases 样例包和 DATA-TRACE 共同构成可审查材料。代码仓库 `MorningStar0709/control-sci` 按 CC-BY-4.0 许可开放；本报告使用的公开医学文献、索引产物与评测 trace 均保留本地复核路径，患者级私有资料不作为公开数据集发布。
