...
![系统执行流程](shared/assets/system_architecture.png)
...
#### 1.3.1 云端 + 本地隐私边界：ResourceScheduler 的硬约束
...
| 验证项 | 命令 / 路径 | 预期输出 | 耗时 |
|:---|:---|:---|:---:|
| Agent 能力注册 | `.\run\run_reviewer_demo.ps1 -Track 2` | intent 注册 / dry-run 计划验证通过 | ~2 s |
...
| 失败恢复证据 | `demo-local-gpu-fallback-20260509-0116.md` | Ollama 不可用时 62 ms 降级到 script | 文档检查 |
...
362 篇控制科学文档——23 本教材与 339 篇 arXiv 论文，合计约 1.3 GB 的 PDF 原始文件，含 253,012 条 LaTeX 公式（密度约每篇 700 条）——构成对文档解析引擎的严苛测试场景。MinerU 在此场景下完成了零系统性失败的全量解析（磁盘 glob 口径），由 Agent 自主编排了整个解析流程，产出以下关键语料指标：
...
![多格式结构化输出](shared/assets/multi_format_demo.png)
...
![公式识别对比](shared/assets/mineru_1.2b_vs_mimo_formula.png)
...
![Agent 执行协议](shared/assets/agent_execution_protocol.png)
...
![Sciverse U 形流水线：三赛道端到端](shared/assets/sciverse_pipeline.png)
...
![Agent Reliability 五维证据矩阵](shared/assets/track2_agent_reliability_matrix.png)
...
![故障注入恢复矩阵](shared/assets/track2_failure_recovery_matrix.png)
...
![多源选择 A/B](shared/assets/track2_source_selection_ablation.png)
...
![资源调度 Pareto](shared/assets/track2_resource_pareto.png)
...
![复杂文档压力覆盖](shared/assets/track2_hard_document_stress.png)
...
![四轨推理与数据边界](shared/assets/agent_four_track_boundary.png)
...
Step 2 [完成]: 62 ms 完成降级切换，输出质量报告
```

**调用工具**：resource_scheduler.py（降级链 `api → ollama → script`）

**最终输出**：62 ms 完成降级，未出现数据级丢失。三层容错架构经三个真实场景验证：Ollama 未启动（Provider 级）、Verifier 低分（Step 级，自改进参数调优后通过）、MinerU 单文件异常（File 级，try/except/continue 跳过）。
...
|:----:|------|:---:|-----------|:----:|
| A | 原始措辞 | 2 | `multi_format_parse` → `quality_arbitrate` | 53 s |
| B | 明确文档路径 | 2 | `multi_format_parse` → `leaderboard_viz` | 26 s |
| C | "审计"关键词 | 1 | `leaderboard_viz` | 15 s |
...
![Sciverse 三源路由决策矩阵](shared/assets/track2_sciverse_source_routing.png)
...
**最终输出**：决策矩阵 5/5 成功执行，源分布为 arXiv:1 / PMC本地:1 / Sciverse:3。5 组查询覆盖了三种不同的路由模式——时效性优先（arXiv 前沿）、隐私约束（PMC 本地）、覆盖广度优先（Sciverse 跨学科）——说明 Agent 的 Intent Router 能够在不同文献源之间根据查询特征做出差异化选择。
...
![双重身份矩阵](shared/assets/dual_role_matrix.png)
...
![本地 Judge 秩相关热力图](shared/assets/judge_spearman_heatmap.png)
...
![本地 Judge 零分率](shared/assets/judge_zero_score_rate.png)
...
![API 共识对齐](shared/assets/judge_api_consensus_alignment.png)
...
![领域偏差热力图](shared/assets/judge_domain_bias_heatmap.png)
...
| 案例 | 层级 | 场景 | 恢复机制 | 结果 |
|:----|:----:|:-----|:--------|:----:|
| Ollama 未启动 | Provider | HTTP 502 Connection Refused | `get_fallback()` → script 降级 | 62 ms 完成 |
...
| Step | 操作 | 工具 | 耗时 | 输入→输出 |
|:---:|:---|:---|:---:|:---|
| 301 | 题目加载 | build_benchmark.py | 0.2 s | 360 题 → 360 题 |
| 302 | Embedding 快速通道 | qwen3-embedding:4b | 1.9 s | 360 → 48 通过 + 12 不通过 + 300 待仲裁 |
| 303 | LLM 第一轮仲裁 | deepseek-v4-flash | 142 s | 300 → 192 通过 + 75 否决 + 3 失败 + 30 待定 |
| 304 | LLM 第二轮仲裁 | deepseek-v4-flash | 38 s | 75 → 75 否决 |
| 305 | 孤儿回收 | deepseek-v4-flash（单答案评估） | 540 s | 185 → 114 回收 + 71 否决 |
| 306 | 最终合并 | build_benchmark.py | 0.3 s | 候选池 → core 500 + full 889 |
...
| 部署文档 | 完整部署与运行说明（`track2_agent_deploy.md`） | 随包文档 |
...
| Step | 操作 | 耗时 | 瓶颈类型 |
|:---|:---|:---:|:---|
| arxiv_search | arXiv API 检索下载 | ~45s | 网络 I/O |
| corpus_parse | MinerU 5 篇 PDF 解析 | ~180s | GPU 计算 |
| benchmark_build | 三 Provider 并行出题 | ~60s | API 并发 |
| quality_arbitrate | Embedding 快筛 + LLM 仲裁 | ~30s | API 串行 |
| model_evaluate | deepseek-v4-flash 评测 | ~70s | API 串行 |
| leaderboard_viz | 排行榜生成 | <1s | CPU |
...
- **目标层（人类负责）**：四维评测体系（A/B/C/D 各 125 题的设计原则）、15 个 Agent Intent 的能力边界、医学章节本体与任务目标
...
![Community Star 社区认证](shared/assets/community_star.jpg)
...
| 2 | 25 题抽样验证（含失败恢复、降级案例）| `track2_agent_20_cases.md`，逐题对照 Intent / Agent Plan / Tool calls / Output / Recovery flow | 每题标注相关证据项 | 5 min |
...
| 19 | 飞轮耗时 391s | `benchmark/agent/agent_report.md` 与 `track2_agent_20_cases.md` 记录该值来自 CLI 输出；`benchmark/agent/logs/demo-data-flywheel.json`、`benchmark/dataset/flywheel_filtered.json`、`benchmark/eval/results/flywheel_eval_deepseek.json` 可交叉验证执行产物完整 | 工程实测记录值；stdout/stderr 未单独归档 |
...
| FR-12 | 步骤恢复 | Self-improve safety net | 中间步骤失败或输出不满足 schema | 进入可审计修正路径，保留原始失败与修正记录 | `track2_agent_20_cases.md` |
| FR-13 | 文件级恢复 | MinerU parse skip | 单个脏文件解析失败 | 跳过失败文件并继续处理其余输入 | `track2_agent_20_cases.md` |
...
| `track2_agent_20_cases.md` | 代表案例摘要 | 25 个案例覆盖 Intent 识别、DAG 规划、工具调用、产物输出、失败恢复与降级路径 |
| `shared/DATA-TRACE.md` | 数据溯源摘要 | 报告关键数字均有权威源文件与复现命令；本报告附录 A 已摘录 35 条核心条目 |
| `data_trace_bundle/manifest.json` | 产物清单摘要 | 汇总报告、日志、图表、样例和验收包，便于离线复核 |
