...
const completeRows = [
  ...
  ['复现方式', 'DATA-TRACE、trace JSONL、CLI、报告附录与完整工作台复核'],
  ...
  return (
    <SectionWrapper>
      <div className="custom-screen space-y-6">
        <PageHeader title="在线展示边界" desc="把在线展示版与完整系统分清楚：云端用于公开体验和可追溯回放，完整系统负责私有资产、长任务、索引模型和批量实验。" />

        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <BoundaryCard title="在线展示版" rows={cloudRows} />
        ...
        <section className="rounded-lg border bg-amber-50 p-5 text-sm leading-7 text-amber-900">
          <strong>展示说明：</strong>在线展示版不是第二套系统，也不是把所有能力搬到公网。它把完整系统中适合公开访问的动作整理成可操作页面，并用 trace 回放呈现重资产能力在受控环境中的完成结果。
        </section>
      </div>
    </SectionWrapper>
  );
