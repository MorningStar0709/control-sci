...
function offlineEvidence(path, query) {
  return {
    query: query?.q || '',
    count: 0,
    results: [],
...
export default function handler(req, res) {
  return new Promise(resolve => {
    ...
    const targetUrl = new URL(`${BACKEND}/api/evidence/${path}${queryString(req.query)}`);
    ...
    request.on('error', () => {
      if (!res.headersSent) res.status(200).json(offlineEvidence(path, req.query));
      resolve();
    });
    req.pipe(request);
  });
