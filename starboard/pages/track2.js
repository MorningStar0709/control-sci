...
const executionProtocol = [
  ['Intent Router', '自然语言目标转成可组合 intent 序列'],
  ['ResourceScheduler', '按 data_policy 选择 API / Ollama / vLLM / Script'],
  ['Executor + Verifier', '执行、检查、重试、降级和写入日志'],
  ['LogStep + Sources', '输出摘要、来源产物和可复现命令'],
];
  ...
  const executeWithQuery = useCallback(async function executeWithQuery(task) {
    ...
    try {
      const response = await startApiTask(action.path, {
        ...
        local_model: runtimeConfig.local_model,
      ...
      <div className="custom-screen space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-4">
            <StepCard index="A" title="任务模板" desc="选择一个已验证任务，也可以手动输入。" status={query ? 'done' : 'pending'}>
            ...
            <StepCard index="B" title="能力注册表" desc="来自 agent_capabilities.json。" status={capabilities ? 'done' : 'pending'}>
              ...
              <p className="text-[11px] text-gray-500 mt-3">公开/脱敏规划任务可调用 API；原文、医疗、chunk、微调产物保持本地。</p>
            </StepCard>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <StepCard
              index="1"
              title="意图路由"
              desc="把任务目标转成可执行 intent 序列。"
              status={result ? (result.status === 'ok' ? 'done' : 'failed') : loading ? 'running' : 'pending'}
              actions={(
                <div className="flex flex-wrap gap-2">
                  <button onClick={execute} disabled={!query.trim() || loading} className="px-4 py-2 bg-gray-900 text-white text-xs rounded disabled:opacity-50">{loading ? '核验中' : currentAction.button}</button>
                  <button onClick={validateChain} disabled={loading} className="px-4 py-2 bg-sky-600 text-white text-xs rounded disabled:opacity-50">{loading ? '核验中' : '核验 Agent 协议'}</button>
                </div>
              )}
            >
              <textarea value={query} onChange={e => patchSession({ query: e.target.value })} rows={3} placeholder="输入任务目标，例如：校验 Medical RAG 索引完整性并输出统计" className="w-full border rounded-lg px-3 py-2 text-sm" />
              ...
              {!result && !loading && (
                <div className="mt-3 text-xs text-gray-500">已加载任务模板和能力注册表。模板按钮默认核验既有产物；长任务只保留可复现命令。</div>
              )}
              {result?.status === 'failed' && (
                <div className="mt-3">
                  <ErrorCallout message={result.message || 'Agent CLI 返回失败'} hint="这通常意味着前端调用链没有使用通过终端验证的 Python 环境；执行计划页仍会保留已验证产物入口。" />
                </div>
              )}
            </StepCard>
              ...
              {loading && !result ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                    ...
                    <div className="border rounded p-2 bg-gray-50"><div className="font-mono">{runtimeConfig.local_model}</div><div className="text-gray-400">Local model</div></div>
...
function resolveTrack2Action(templateId, text) {
  ...
  const actions = {
    flywheel: {
      ...
      explain: '默认只核验已沉淀产物，不调用出题或评测 API；完整飞轮保留可复现命令。',
    ...
    check_index: {
      ...
      button: '核验索引来源',
      label: '来源核验：manifest / FAISS / BM25 来源路径',
      explain: '只核验既有索引产物和来源路径，不重建向量索引。',
      status: 'artifact_reuse',
      loadingSteps: ['读取 manifest', '检查 FAISS', '检查 BM25'],
      timeoutMs: 45000,
      live: false,
    },
    evidence_bundle: {
  ...
  return actions[id] || {
    ...
    button: '生成计划',
    label: '规划模式：只生成 DAG，不执行长任务',
    explain: '只生成计划，不执行真实长任务。',
    status: 'planned',
...
function getResultModeMeta(result) {
  ...
  if (mode === 'live_smoke') {
    return {
      status: 'live_execute',
      label: '专项执行',
      explain: '本次调用了出题/评测链路，并把新结果写入 _scratch；为避免等待，只跑 1 个 chunk 与 1 道评测题。',
    };
  }
  if (mode === 'live_visual_audit') {
    return {
      status: 'live_execute',
      label: '专项执行',
      explain: '本次调用视觉审计模型处理 1 条样本；速度快是因为样本数被限制为 1。',
    };
  }
  if (mode === 'live_build') {
    return {
  ...
  if (mode === 'live_check' || mode === 'live_sample') {
    return {
      status: 'live_check',
      label: '专项检查',
      explain: '本次读取本地文件并统计/抽样；没有重建索引或重新评测全量模型，所以会很快。',
    };
  }
