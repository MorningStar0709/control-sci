const REPLAY_CASES = [
  {
    ...
    claims: [
      ...
      claim('部分闭环系统研究显示目标范围内时间改善，并伴随低血糖风险指标下降或维持较低水平。', ['PMC6448906_chunk_010', 'PMC7618672_chunk_005']),
  ...
  {
    match: ['主要终点', '安全性'],
    answer: '根据已保存的文献来源摘要，不同研究对主要终点的定义不同：例如 R0/R1 切除后的总生存期、随机化后 24 小时内药物浓度目标达成率等。安全性信息通常作为次要终点呈现，包括不良事件类型、频率和严重程度，或特定死亡率/肾衰竭等安全结局。',
    ...
    claims: [
      ...
      claim('安全性信息常通过不良事件类型、频率和严重程度等指标呈现。', ['PMC8600696_chunk_007']),
    ],
    evidence: [
  ...
  {
    match: ['纳入排除', '纳入', '排除'],
    answer: '根据已保存的文献来源摘要，随机对照试验的纳入/排除标准用于筛选合格受试者。来源示例包括参与者选择和样本量阶段应用具体标准；排除标准可包括无法及时提供知情同意、妊娠、参与其他干预试验、已接受特定治疗超过规定时间、ECMO 等情形。',
...
const MECHANISM_REPLAY_HINTS = [
  {
    match: ['为什么'],
    answerPrefix: '这个问题需要机制解释，而不是只复述结论。基于已保存的本地来源，回答会按“结论 → 可能机制 → 适用边界 → 安全声明”组织；如果来源不能支撑机制，会明确说明依据不足。\n\n',
  },
  {
    match: ['机制'],
    answerPrefix: '这个问题需要机制解释，而不是只复述结论。基于已保存的本地来源，回答会按“结论 → 可能机制 → 适用边界 → 安全声明”组织；如果来源不能支撑机制，会明确说明依据不足。\n\n',
  ...
  {
    match: ['原因'],
    answerPrefix: '这个问题需要原因解释，而不是只复述结论。基于已保存的本地来源，回答会按“结论 → 可能机制 → 适用边界 → 安全声明”组织；如果来源不能支撑机制，会明确说明依据不足。\n\n',
  },
];
...
export function findMedicalAskReplay(question) {
  ...
  return {
    ...
    limitations: '该结果来自已保存的可复现实验 trace，用于展示；手动输入问题仍可走真实本地 RAG。',
    abstain: false,
    abstain_reason: '',
