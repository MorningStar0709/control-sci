...
[System.Environment]::SetEnvironmentVariable("OPENAI_API_KEY", "<DEEPSEEK_API_KEY>", "User")

# MiMo-V2.5 视觉 + MiMo 文本（跨模态审计 + 补充生成）
[System.Environment]::SetEnvironmentVariable("MIMO_API_KEY", "<MIMO_API_KEY>", "User")

# MiniMax（补充生成）
[System.Environment]::SetEnvironmentVariable("MINIMAX_API_KEY", "<MINIMAX_API_KEY>", "User")
...
对应 intent `quality_arbitrate`，使用 DeepSeek 进行 Embedding 快速通道 + LLM 深度仲裁：
...
生成 → needs_review → [Embedding 快速通道] → auto_passed
                   └→ [LLM 深度仲裁] → reviewed_kept / reviewed_discarded
```
...
conda run --no-capture-output -n myenv python benchmark/pipeline/export_dataset.py `
  --input benchmark/dataset/core.json --output huggingface/control-sci-corpus `
  --push-to-hub "<HF_USERNAME>/control-sci-benchmark" --hf-token "<HF_TOKEN>"
```
