...
- 云端入口：https://demo.askiler.com/
- HTTP 入口：http://demo.askiler.com/（会自动跳转到 HTTPS）
- 前端访问码：`ControlMind@2026`

打开页面后，在顶部状态栏的“访问码”输入框填入访问码并点击“保存”。访问码只用于解锁公开云端实时调用；云端 Demo 仅处理公开 PDF、公开 URL 或脱敏问题，不接收私有文档、患者材料或内部索引。
...
`_scratch/` 是本地临时输出目录，可删除、可重建，不属于权威数据源。最小验收路径默认复用提交包内已沉淀的公开 PDF、Markdown、题库、索引和 evidence；arXiv/PMC 在线下载保留为语料扩展能力，不作为复现前置条件。

---

## 三、提交包结构

ControlMind 同时提交三条赛道。本文件是快速导航，三份主报告是完整技术说明，三份“20+ 案例包”是样例级证据索引。

| 目标 | 建议入口 |
|:---|:---|
| 快速查看三赛道样例 | `python demo/cli/controlscidemo all --quick` |
| 抽查 Track1 科学对齐 | [track1_sci_align_20_cases.md](shared/track1_sci_align_20_cases.md) |
| 抽查 Track2 Agent 执行 | [track2_agent_20_cases.md](shared/track2_agent_20_cases.md) |
| 抽查 Track3 医疗 RAG | [track3_medical_rag_20_cases.md](shared/track3_medical_rag_20_cases.md) |
...
以下入口命令均已于 2026-05-15 通过完整验收——数据集结构校验、API/Judge 真实评测、RAG 检索、容器化部署一键检查均在产出侧印证。2026-05-17 又补充了四项轻量验收：三赛道最小入口、Track1 本地 JSON 加载、Track2 本地 dry-run DAG、Track3 安全拒答边界。具体记录见 [minimal_repro_results.md](shared/minimal_repro_results.md) 与 `data_trace_bundle/12_final_supplemental_experiments/`。
...
| Track2 Agent | 将科学文档语料生产从脚本流水线提升为可规划、可恢复、可审计的 Agent 执行协议 | 14 Intent、资源调度器、失败恢复案例、结构化日志、CLI replay | `shared/track2_agent_20_cases.md`、`controlscidemo track2 --quick` | 复杂文档理解、Agent 规划执行、稳定性、工程复现 |
...
| Track1 | 工程质量 / 可复现 | [minimal_repro_results.md](shared/minimal_repro_results.md)；[DATA-TRACE.md](shared/DATA-TRACE.md)；`data_trace_bundle/manifest.json` | 0.9 秒结构验证、排行榜原始结果、manifest SHA-256 溯源 |
...
| Track2 | 难点场景 / 创新性 | [track2_agent_report.md](track2_agent_report.md) §3-§4；`data_trace_bundle/07_flywheel/agent_capabilities.json` | 14 Intent、四类 provider、data_policy、跨赛道能力注册 |
...
| Track2 | 稳定性 / 工程复现 | [track2_agent_report.md](track2_agent_report.md) §5.4-§5.6；[minimal_repro_results.md](shared/minimal_repro_results.md)；[track2_agent_deploy.md](shared/track2_agent_deploy.md)；`data_trace_bundle/12_final_supplemental_experiments/track2_fallback/` | fallback、dry-run、8 步本地 DAG、资源调度健康检查、部署边界 |
| Track2 | 开源共享 / 产业生态 | [track2_agent_report.md](track2_agent_report.md) §7 | GitHub 仓库、HuggingFace 数据、Apache 2.0、可复用 Agent 协议 |
...
| Track3 | 工程实现 / 可用性 | [track3_medical_rag_report.md](track3_medical_rag_report.md) §5.2、§6；[track3_medical_deploy.md](shared/track3_medical_deploy.md) | REST API、Docker Compose、CLI replay、本地索引与私有部署路径 |
...
| Intent 注册 | 查看报告 §3 或运行资源调度器命令 | 14 Intent 与 provider 映射 | 复杂任务拆解 |
...
python benchmark\agent\agent_cli.py --dry-run --local --intents medical_rag,cross_modal_audit,leaderboard_viz
...
conda run -n myenv python -c "import json; d=json.load(open('benchmark/eval/results/leaderboard.json',encoding='utf-8')); [print(m['model'], m['overall_score']) for m in d['models']]"
...
# Track3: 医学视觉描述数量
conda run -n myenv python -c "print(sum(1 for _ in open('data/sources_medical/vision/vision_descriptions.jsonl',encoding='utf-8')))"
```
