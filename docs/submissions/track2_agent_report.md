...
> **版本**：v3.1（润色版） | **更新日期**：2026-05-14
> **所属**：2026 MinerU 挑战赛 · 智能体与数据工程赛道
> **数据追溯**：本报告中所有定量声明均指向 `shared/DATA-TRACE.md` 唯一权威条目，每个数字可独立复现

> **最小真实闭环验证**：2026-05-14 已完成 Agent dry-run、能力注册校验、10 题真实评测、leaderboard 生成与 Medical RAG API health/search 联合验证。详见 [minimal_repro_results.md](minimal_repro_results.md)。
...
三条技术趋势的交汇使垂直领域自主化语料处理成为可能。其一，LLM 在通用基准上趋于饱和，垂直领域的深度评测成为区分模型能力的新标尺——控制科学作为 AGI4S 的基础数学语言，其数据处理需求在此刻尤为迫切。其二，MinerU 等新一代文档解析引擎使大规模科学文献的结构化处理首次具备工程可行性——362 篇文档（含 253,012 条 LaTeX 公式）的全量解析不再需要人工逐篇调参。其三，消费级 GPU（RTX 5090, 24GB）使垂直领域的全栈 AI 工作流不再依赖云计算——一台笔记本即可完成从解析到训练到评测的完整闭环。这三条趋势的交叉点上，"消费级硬件 × AI 最大化执行 × 垂直领域深评测"的技术可行性得到实际验证。Agent 的全部下游任务——跨模态审计、题目生成、评测执行——均建立在 MinerU 的结构化输出之上，而本地 Qwen3.5 模型家族在同一张显卡上覆盖了从 35B 推理到 9B 视觉描述的全部 AI 能力。

本系统采用 **AI 驱动最大化（AI-driven Maximalism）** 作为技术原则：人类定义目标、边界和评价协议，Agent 最大化承担可协议化、可日志化、可复现的执行层决策。换言之，项目并不主张人类完全退出系统；相反，它将人类决策集中在"做什么、为什么做、什么算合格"的高价值层，把检索、解析、调度、审计、评测、恢复和报告生成等重复执行环节交给 Agent。
...
| 评测执行 | 人工批改 → 争议讨论 → 重新打分 | LLM-as-Judge → 三 Judge 交叉验证 → 一致性报告 |
| 排行榜更新 | 手动统计 → 手动排版 → 手动发布 | Agent 自主更新 500→515 题 → 自动 HTML 排行榜 |
| 领域迁移 | 为每个新领域重写解析/切片/评测代码 | 同一 Agent 框架 → 核心模块零代码改动 → 从控制科学到医学 |

> 数据来源：人工基线描述基于传统学术文献处理流程经验估计；AI 实现部分各项数字来源见附录 A 对应条目。

若将这 7 个环节的人工基线量化为具体的工时数：文献检索——逐篇阅读标题与摘要判断相关性、手动下载 PDF，362 篇论文约需 40 工时；文档解析——逐篇调整 MinerU 参数、监控 GPU 显存占用、处理 PDF 解析失败后的手动重启，约需 120 工时；质量审计——4,996 个图片-公式共现 chunk 逐张比照 LaTeX 输出与原始图片是否语义一致，按每张图 3 分钟估算约 250 工时；题目生成——人工设计 500 道四维评测题目并经过同行审阅修改，约需 80 工时；评测执行——逐题阅读 9 个模型的答案并评分，500 题 × 9 模型 = 4,500 次批改，约需 60 工时；排行榜更新——手动统计各模型维度分、排版 HTML 排行榜，约需 8 工时。**累计估算约 558 工时**——相当于一名全职研究助理连续工作 14 周的产出。而这仅是构建与评测一个领域（控制科学）的语料库所需的人力；若将领域迁移到医学并重新执行完整流程，人工工时将再次翻倍。Agent 将这些重复执行成本压缩到最低——从解析到评测的执行环节按固定协议自动运行，并通过结构化日志保留可审计轨迹。

这条从文献到排行榜的全链路中，人类定义评测体系、Agent Intent 规范和质量约束；一旦协议固定，从文献检索到评测报告的执行链路由 AI Agent 自动运行。这是一个精确的边界界定——目标层由人工定义，约束层由人工设定并由 AI 校验，执行层（从解析到评测的每一个可日志化步骤）由 AI 最大化承担。

### 1.2 系统概述

ControlSci Data Agent 是一个面向科学文档跨模态语料处理的智能体系统，将 MinerU 文档解析引擎、MiMo-V2.5 原生视觉模型、13 Intent 自主编排和 QLoRA 领域微调整合为一个从语料解析到质量审计到评测闭环的完整系统。系统已在 362 篇控制科学文档上完成验证，并在医学文献领域实现了跨领域零代码迁移。

**一句话贡献**：ControlSci Data Agent 的核心贡献是把科学文档语料生产从脚本流水线提升为可规划、可恢复、可审计的 Agent 执行协议。
...
用户输入 → Intent Router (DeepSeek v4-flash few-shot)
        ...
        自动选择最合适的 provider / 自动降级
              │
      ┌───────┼───────┬───────┐      ← 四轨推理引擎
      ▼       ▼       ▼       ▼
  轨道1: API  轨道2: Ollama  轨道3: vLLM  轨道4: Ollama Vision
  DeepSeek    qwen3.5:9b    MinerU-1.2B  qwen3.5:9b
  MiMo-V2.5   gemma3:4b     公式识别     医学图片描述
  (远端)      qwen3.5:35b   (本地专项)   (本地隐私)
              (本地隐私)
      │       │       │       │
      └───────┼───────┴───────┘
              ▼
   ...
   (闭环自修正 + 三层降级恢复)
              ▼
   Output: execution_log.json + deliverables
```

轨道 4 使用与文本推理相同的 qwen3.5:9b 权重——unified vision-language foundation 使同一模型同时承载文本推理与医学图片描述两轨任务。该轨道已在 730 张医学图片（92 篇 PMC 文献）上完成全量验证，单图均值 16.6s，描述 818 字符，检索效果与 MiMo-V2.5 API 可比（详见赛道三 §4.3 AB 对比）。作为对照实验，qwen3.5:4b 在 30 张含统计数字的医学图表上几乎无法产出结构化数值（均值 0.0/图 vs 9B 的 5.4/图）——不是精度不足，是能力断崖。这一发现从反面验证了四轨引擎的选型判断：视觉轨道选择 9B 而非更小的本地模型，不是显存分配的结果，而是能力边界的测绘结果。

![Figure 1.1](shared/assets/system_architecture.png)
*Figure 1.1: ControlSci Data Agent 系统架构。五层流转：Input（CLI / API / NL）→ Routing（IntentRouter + ResourceScheduler）→ Inference（四轨推理引擎：API / Ollama / vLLM / Ollama Vision）→ Execution（Executor + Verifier 闭环自修正 + 三层容错降级）→ Output（JSON / Leaderboard / Corpus）。Single RTX 5090 全栈部署。*

#### 云端 + 本地隐私边界：ResourceScheduler 的硬约束

四轨推理引擎的核心不是“能用 API 也能用本地”，而是按数据风险分层：**公开/脱敏任务上云，原文/医疗/微调/中间 chunk 留本地**。这条原则由 `resource_scheduler.py` 和 `agent_capabilities.json` 共同固化，而不是报告层口号。
...
| `mineru_parse` / `corpus_parse` | local_only | MinerU 本地 API / script | 否 | 原始 PDF、图片、Markdown 原文和 chunk 不出本机 |
| `multi_format_parse` | local_only | MinerU 本地 API | 否 | 多格式原文解析属于本地数据处理边界 |
...
因此，本系统的“AI 驱动最大化”有两条同时成立的约束：执行层尽可能交给 Agent；数据边界不因自动化而放松。Agent 可以自动选择 provider，但不能把 `local_only` intent 路由到云端。这使得同一套代码既能在公开评测场景使用强 API 提高审阅上限，也能在医院、企业内网或离线环境中切换为本地隐私模式。

#### 为什么这不是普通 pipeline

本系统不是把若干脚本顺序串起来，而是将科学文档语料生产抽象为一套可组合、可恢复、可审计的 Agent 执行协议。普通 pipeline 的核心是固定流程；ControlSci Data Agent 的核心是：自然语言输入被解析为 intent 序列，每个 intent 被拆解为可验证 step，ResourceScheduler 在 API / Ollama / vLLM / Script 之间做 provider 路由，Executor 执行后由 Verifier 判断是否通过、重试、降级或追加修正步骤，最终以统一 LogStep schema 落盘。

换言之，pipeline 只记录"流程应该怎么走"，而 Agent 在协议内决定"当前条件下应该调用哪个工具、失败后如何恢复、输出是否满足质量约束"。D 数据飞轮、Ollama 未启动自动降级、跨模态审计断点续跑、QLoRA 训练后自动评测这四类案例分别对应规划、恢复、审计和验证四个 Agent 能力面，而不是单一脚本链条的线性执行。

#### 从 legacy pipeline 到隐私感知 Intent Agent

项目早期的 `benchmark/agent/agent.py` 是固定流水线 Agent：它将 benchmark build、quality arbitrate、split dataset、model evaluate、summarize 和 report visualization 串联起来，证明了“从数据构建到排行榜生成”的自动化调用链可执行。这个版本不是废弃成果，而是当前 Agent 的 legacy baseline：dry-run、step timeout、subprocess 编排、ExecutionLog / LogStep 与 pipeline summary 等工程经验，都被后续 `agent_cli.py` 继承并扩展。

随着系统需求从“固定流程可跑”升级为“14 intent 可组合、四路径资源调度、隐私边界可审计、云端/本地 profile 可切换”，正式入口统一迁移到 `agent_cli.py`。当前 `agent.py` 保留为 compatibility wrapper：历史命令仍可运行，但会转发到 `agent_cli.py`，避免旧实现绕过 `resource_scheduler.py` 的 `data_policy`、`agent_capabilities.json` 的 intent registry 和最新日志策略。完整演进说明见 [track2_agent_legacy_baseline.md](track2_agent_legacy_baseline.md)。

这条演进路径也解释了本系统为何不是临时拼接的脚本集合：先有固定流水线证明最小可行闭环，再抽象出 intent、scheduler、verifier 和 data policy，最终形成可解释、可审计、可迁移的 Agent 协议。

### 1.3 差异化定位

以下五个差异化锚点构成系统在赛道二评分体系中的核心优势——每个锚点对应一条关键评分维度的最高权重考察项：

| 锚点 | 核心优势 | 评分维度 |
|:-----|:--------|:-------:|
| 真实工业级控制科学语料 | 362 篇全量 MinerU 解析，28,514 chunks / 253K 公式 / 4,996 共现 | D1（复杂文档理解）|
| MiMo-V2.5 原生视觉 × 跨模态审计 | 单模型完成"看图→理解→比照 LaTeX→判决"全链路，9,207 条自动判决 | D2（难点攻克）|
| 13 Intent × 四轨推理引擎 | 自然语言 → 多步 plan → 三阶段流转 → Verify 闭环 | D3（任务规划）|
| 四路径调度 + 三层容错 | API/Ollama/vLLM/OllamaVision 自动感知 + 自动降级 + Checkpoint 恢复 | D4（系统稳定性）|
| 双赛道联动 + 跨领域零代码迁移 | 同一仓库同源资产驱动两个赛道，控制科学↔医学 17 核心模块零改动 | D5（生态价值）|

### 1.4 报告结构与评分维度对照

以下为报告各章节与赛道二 5 维评分细则的对应关系，供评审专家组快速定位：

| 评分维度 | 满分 | 对应章节 | 关键证据 |
|:---------|:---:|:---------|:--------|
| 复杂文档理解与结构化处理能力 | 20 | §2 全章 | 28,514 chunk 磁盘实存、4,996 共现 chunk 全量审计、跨模态对齐 79.3%、嵌入四维验证、赛题六大痛点逐条对标 |
| 难点场景攻克与技术创新性 | 15 | §4 全章 | 评分者反规模定律 L1-L4、QLoRA 反直觉发现链（C维幸存/PPL悖论/灾难性遗忘）、MiMo-V2.5↔MinerU 1.2B 公式识别对比、think on/off 对照实验 |
| Agent 任务规划与自动执行能力 | 30 | §3 全章 | 13 Intent 能力矩阵、四轨推理引擎、D 数据飞轮 391s 6-step 闭环、跨模态全量审计 9,207 条自动判决、QLoRA 自动化微调闭环、闭环自修正 50% 成功率 |
| 系统稳定性与工程可复现性 | 20 | §5 全章 + §4.1 | 三层容错经验证 3 场景、跨管道可复现 MAE=0.0003、故障恢复矩阵 6 场景、三模式部署、部署文档 708 行、一键复现脚本 |
| 代码开源共享与产业生态价值 | 15 | §6 全章 + §7.3 | GitHub 开源四模块、HF 数据集四子集、双赛道联动 6 资产矩阵、3 自研 Trae Skill 社区发布、跨领域零代码迁移、Community Star 认证 |
...
| 跨管道复现 | `benchmark/eval/results/analysis/cross_pipeline_reproducibility.md` | MAE=0.0003，排名一致 | CPU 读取 |
| 失败恢复证据 | `demo-local-gpu-fallback-20260509-0116.md` | Ollama 不可用时 62ms 降级到 script | 文档检查 |

本报告所有量化数字指向统一溯源文件 `shared/DATA-TRACE.md`。各章节内容安排如下：§2 展示语料处理与多格式解析能力，§3 阐述 Agent 的任务规划与自主决策机制（含 6 个完整执行案例），§4 分析实验发现（跨管道可复现、反规模定律、QLoRA 反直觉链、PPL 悖论与公式识别对比），§5 描述工程实现与系统稳定性，§6 讨论跨领域迁移、双赛道联动与方法论通用性。

---

## §2 数据构建与 MinerU 集成

### 2.1 语料规模：Agent 能力的产出证明
...
> 数据来源：语料元数据索引（文档计数）+ 多模态共现统计（chunk 及共现口径）。完整路径见附录A。

14 个子领域覆盖：PID 控制、估计与定位、导航制导与控制、控制理论、数字控制、智能控制、最优控制、现代控制、线性系统、经典控制、自抗扰控制、自适应控制、非线性控制、鲁棒控制。

#### 2.1.1 赛题六大痛点对标

赛道二核心挑战定义了六大文档处理难点。下表按赛题原文对每个痛点给出本系统的 Agent 策略与量化证据所在章节：

| 赛题痛点 | Agent 策略 | 量化证据章节 |
|:---------|:----------|:-----------|
...
| **标准解析问题** — 基于规范或行业标准的解析 | Agent 13 Intent 编排 + 跨领域零代码迁移验证（控制科学 → 医学，17 模块零改动） | §3（13 Intent 能力矩阵）+ §6.1（跨领域迁移） |

> 数据来源：赛题痛点原文取自 2026 MinerU 挑战赛赛道二核心挑战定义；Agent 策略来自本系统架构设计；量化证据全部指向本报告内已有数据的对应章节。

### 2.2 多格式解析能力

Agent 通过 `mineru_parse` 和 `multi_format_parse` 两个 intent 覆盖五种文档格式的统一结构化处理。PDF（扫描版和数字版）、PPTX、DOCX、XLSX 和 PNG 扫描件全部由同一管线处理为 chunk 级 Markdown + 独立图片文件 + LaTeX 公式源码。四种非 PDF 格式的解析效果如下（基于 `benchmark/agent/test_materials/` 真实产出）：

![Figure 2.1](shared/assets/multi_format_demo.png)
*Figure 2.1: 四种格式的 MinerU 统一结构化输出对比。数据来源：Agent 多格式解析测试集。生成脚本：multi_format_demo.png。*
...
四种格式的解析产出全部遵循同一结构化 schema，使下游的题目生成、质量审计和嵌入分析可统一处理，无需针对源格式做适配。HTML 格式因 MinerU 官方不支持，不在 `multi_format_parse` 的覆盖范围内——Agent 不做 MinerU 不支持的能力声明。
...
| 一致率（完全一致） | 75.9% | MiMo-V2.5 判决 |
| 综合对齐质量 | 79.3%（完全一致 + 部分一致） | MiniMax-VL 交叉验证 |
...
MiMo-V2.5 在编辑距离和 BLEU 上领先，但 MinerU 1.2B 在字符匹配率上反超——1/258 参数量的专用模型在字符级细节上比 310B 通用模型更精确。这一对比印证了四轨引擎的设计合理性：通用视觉模型适合语义理解任务，专用文档模型适合格式敏感任务，两者在同一系统中的互补协同比单一模型覆盖所有场景更高效。

![Figure 2.2](shared/assets/mineru_1.2b_vs_mimo_formula.png)
*Figure 2.2: 1.2B 专模 vs 310B 通用模型的公式转录质量（42 条公式）。数据来源：MinerU 1.2B VLM 公式识别对比分析。生成脚本：mineru_1.2b_vs_mimo_formula.png。*

---

## §3 方法

13 Intent 的定义不绑定任何特定领域——它们覆盖的是"检索→解析→结构化→审计→评测→报告"这一科学文献处理的通用范式。在后续实验中（§6.1），同一套 intent 规范和编排逻辑在医学文献场景下以零代码改动完成了从检索到评测的全链路迁移。

### 3.1 13 Intent 能力矩阵

系统定义了 13 个预定义 intent，覆盖从文档解析到评测报告生成的全链路。这些 intent 构成 Agent 的可编排工具箱——每个 intent 对应一个可独立执行的原子能力单元：
...
13 个 intent 均在 dispatch_map 中注册，验收测试对全部 intent 的 8 个验证维度（注册检查、分发检查、Schema 检查、依赖检查、Executor Dry-Run、CLI Dry-Run、Router 检查、自然语言解析）全量覆盖通过。

### 3.2 四轨推理引擎

Agent 在同一台 RTX 5090 上部署了四轨推理引擎，由 ResourceScheduler 按任务类型自动路由：

| 轨道 | 引擎 | 部署位置 | 适用任务 | 延迟 |
|:----|:-----|:--------|:--------|:---:|
| 轨道1: API | DeepSeek v4-flash / MiMo-V2.5 | 远端 | 通用文本推理、视觉审计、Deep Judge | ~1-5s |
| 轨道2: Ollama | qwen3.5:4b/9b/35b / gemma3:4b | 本地宿主机 | 隐私文档、离线评测、本地 Judge | ~0.5-1.5s |
| 轨道3: vLLM | MinerU-1.2B VLM | WSL2 容器内 | 公式识别与对比 | ~0.3-1s |
| 轨道4: Ollama Vision | qwen3.5:9b (视觉) | 本地宿主机 | 医学图片描述、本地隐私视觉理解 | ~1-3s/张 |

四轨在 Windows 宿主机 + WSL2 (vLLM) + Ollama 原生进程的异构环境中实现统一调度。轨道 4 与轨道 2 共享同一 qwen3.5:9b 权重——Ollama 的 unified vision-language foundation 使同一模型同时承载文本推理与视觉描述两轨任务，无需额外模型拉取或显存分配。ResourceScheduler 的 ProviderParameterMap 维护所有轨道 provider 的参数配置，Intent Router 解析出的每个 step 自动匹配最合适的轨道。当主轨道不可用时，按 `api → ollama → script` 链自动降级。
...
> 数据来源：Agent 自我修正实验报告。完整溯源见附录A #32。

### 3.5 六个典型任务执行案例

系统采用统一 LogStep schema，所有 Agent 执行过程——从 Intent Router 的 plan 生成到每个 step 的 provider 选择、耗时与输出状态——均以结构化日志原子落盘。以下六个案例的每个执行细节均直接来自这些可追溯日志，每个案例包含任务输入、执行步骤、调用工具和最终输出结果。
...
**最终输出**：Agent 执行日志 391 秒完成 6-step 全链路，全量执行细节记录在统一 LogStep schema 的日志文件中。

**核心叙事**：整个过程没有人工指定"用哪个模型""每篇出几道题""这篇行不行"——Agent 在每个决策点都有统一 LogStep schema 的日志记录。
...
Step 0 [multi_format_parse]: 格式自动检测
                              ...
                              全部 → 统一 chunk schema + LaTeX 公式 + 独立图片
```

**调用工具**：mineru_to_md.py + MinerU API

**最终输出**：每种格式 ~3 分钟完成解析，产出四种格式全部遵循同一结构化 schema 的 chunk 级数据。下游的 benchmark_build、cross_modal_audit、chunk_embedding_analysis 无需针对源格式做适配。
...
**最终输出**：62ms 完成降级，零数据丢失。三层容错架构经三个真实场景验证：Ollama 未启动（Provider 级）、Verifier 低分（Step 级，自改进参数调优后通过）、MinerU 单文件异常（File 级，try/except/continue 跳过）。

#### 案例 6：开放指令拆解尝试 — 跨报告数据审计

**任务输入**："检查赛道一报告 §4.2 的 9 模型总分与 leaderboard_complete.json 是否一致，逐模型比对，输出差异报告"

**没有对应的预定义 intent**。这是一条涉及跨文档（Markdown 报告 + JSON 数据文件）交叉比对的数据审计指令，未被 13 Intent 矩阵覆盖。为消除单次采样的偶然性，进行了 3 条措辞变体的交叉验证（同一 temperature=0.1，DeepSeek v4-flash）：
...
| 案例 | 数据理解/结构化 | 多步规划 | 工具调用 | 异常恢复 | 日志可追溯 | 对应评分点 |
|:---|:---:|:---:|:---:|:---:|:---:|:---|
| D 数据飞轮 | ✓ | ✓ | ✓ | — | ✓ | 复杂任务规划、自动执行、排行榜更新 |
| 跨模态全量审计 | ✓ | ✓ | ✓ | ✓ | ✓ | 难点场景攻克、图文公式对齐审计 |
| QLoRA 微调闭环 | ✓ | ✓ | ✓ | — | ✓ | 训练-评测闭环、成本/精度权衡 |
| 多格式批处理 | ✓ | ✓ | ✓ | — | ✓ | PDF/PPTX/DOCX/XLSX/PNG 统一结构化 |
| 失败恢复演示 | — | ✓ | ✓ | ✓ | ✓ | Provider 降级、系统稳定性 |
| 开放指令拆解 | ✓ | ✓ | ✓ | — | ✓ | 泛化边界、Router 变异性与改进方向 |

其中"开放指令拆解"不是作为满分能力展示，而是作为边界样例：系统能生成合理计划，但 intent 选择受措辞影响。将这个负面样例纳入报告，是为了让评审看到 Agent 的能力边界和下一步改进路径，而不是只展示成功案例。

### 3.6 编排的数据入口：解析引擎的结构性依赖

**数据入口的结构性依赖**。13 个 Intent 中，`mineru_parse`、`multi_format_parse` 和 `cross_modal_audit` 三个直接依赖 MinerU 的解析输出——但 MinerU 的影响范围由输入 schema 的传递链界定：`corpus_parse`、`benchmark_build`、`quality_arbitrate`、`model_evaluate` 和 `leaderboard_viz` 五个下游 intent 的输入格式完全由 MinerU 的输出 schema 定义。题目生成依赖 chunk 级 Markdown + LaTeX 公式源码，质量审计依赖图片与公式的共现索引，评测依赖题目与 chunk 源文档的双向追溯——所有这些 schema 均由 MinerU 在解析阶段一次性确定，下游模块无需二次转换。当一个工具的输出格式定义了五个下游模块的共同接口时，它在数据架构中不再是可互换的组件，而是所有下游逻辑的共同前提。

**mineru-to-md 五步进化：每一步都是 Agent 应对生产故障**。mineru-to-md Skill 在 362 篇文档的全量解析中经历了五次关键进化：(1) DETACHED_PROCESS 长任务隔离——解决 Windows 子进程生命周期与主脚本耦合导致的僵尸进程；(2) GPU 显存生命周期感知——检测 RTX 5090 24GB 剩余显存，当低于安全阈值时自动暂停解析队列、等待释放后恢复；(3) stats 统计注入审计——每次解析完成后自动生成磁盘级统计，使 Agent 在 `corpus_quality_report` 阶段可直接消费统计数据而无需重新遍历文件系统；(4) 双模式耗时模型——根据 PDF 页数、格式类型（扫描版/数字版）和 GPU 负载动态预估单篇解析用时，为 ResourceScheduler 的任务调度提供先验信息；(5) skip-existing 断点续跑——检测已解析的 chunk 目录完整性，跳过已完成文档，配合 Checkpoint 机制实现零浪费恢复。这五次进化全部由 Agent 在实际编排中自然驱动——每一步都是因为"在生产中遇到问题→Agent 检测到失败→反馈到 Skill 改进"。进化完成后，Agent 不需任何代码修改即可获益于这些改进，因为 Skill 的接口契约（输入 PDF / 输出 chunk）从未改变。

**跨领域验证：同一引擎，零代码迁移**。从控制科学到医学文献的跨领域迁移（§6.1）提供了最有力的结构性证明：迁移涉及的核心模块（`agent_cli.py` 1,464 行、`resource_scheduler.py` 662 行、`visual_audit.py` 520 行）零代码改动。原因不在于 Agent 的"通用性"，而在于控制科学和医学的数据结构共性——两者都由同一解析引擎统一为 chunk 级 Markdown + LaTeX 公式 + 独立图片文件。当上游解析引擎消除了格式差异后，Agent 的编排逻辑面对的始终是同一数据结构——领域迁移的成本从 O(N) 的代码重写降为 O(1) 的数据切换。五个下游 intents 的输入 schema 全部由解析阶段的输出格式定义——解析引擎的格式约束通过输入 schema 的传递链直接定义了 Agent 的能力边界和可迁移范围。
...
**为什么选择 flat JSON 而非数据库？** 文件型 JSON（`core.json` 约 1.1 MB，`full.json` 约 2.1 MB）可通过 HuggingFace 的 `load_dataset` 零适配加载——不需要索引构建、不需要 ETL、不需要依赖注入。验证 500 题仅需 0.9 秒纯 CPU。数据库会增加 AI 系统的消费摩擦，而减少摩擦正是 AI-Ready 数据集的本质要求。
...
ControlSci Data Agent 不仅是语料处理的自动化工具。在自主编排 13 Intent 的全链路执行中，它产出了一个意外的副产品——对 LLM 评测行为的系统性量化观察。
...
**PPL 悖论（-53.6% but 评分 -0.0034）**同样来自 Agent 的全自动评测闭环——Perplexity 探针与 Judge 评分两条独立轨道在 Agent 的编排下自动对齐，暴露了「语言建模进步 ≠ 推理能力进步」的信息论鸿沟。

这三个发现中的任何一个都可以独立构成一篇 workshop paper。Agent 的核心价值不是「跑得更快」——而是它使得人类研究者能够**触及此前无法系统观测的现象**。当 Agent 在一个人不能及的规模（6 模型 × 500 题 × 4 维）上自主执行评测时，它不再是工具——它成为了一台科学发现引擎。
...
MAE=0.0003, Pearson r=1.0000, Spearman ρ=1.0000。两条独立管道的评分偏差均小于 0.001，排名完全一致。这一可复现性验证了评测系统的工程可靠性——数据的结构化程度足以支持无歧义的自动化评测，这是所有后续分析可靠性的底层保障。
...
> 数据来源：交叉 Judge Kappa 统计与 Benchmark 质量审计报告。完整溯源见附录A #29-30。

### 4.2 评分者悖论与反规模定律（L1-L4）

#### 4.2.1 双重身份矩阵视角

ControlSci Benchmark 的 9 个模型在评测中扮演双重身份：既是答题者（接受评分），也是裁判（为同一批答案打分）。对比 8 个 API 模型在 500 题上的全量评分，得到双重身份矩阵：

![Figure 4.1](shared/assets/dual_role_matrix.png)
*Figure 4.1: 双重身份矩阵——8 个 API 模型的答题分与 Judge 均值对比。数据来源：排行榜与 Judge API 评分数据。生成脚本：dual_role_matrix.png。*
...
8 个 API 模型的答题分与 Judge 均值几乎完全一致（Pearson r=0.999991, p<0.001）——模型在"当裁判"时的均值与其"当答题者"时的表现高度相关。但 Qwen3.5-9B 作为在本地评测的模型，其 Judge 分（0.4619，本地评分）显著低于答题分（0.6250，API 评分），Δ=0.1631——答题第三名但裁判最严。这一差异揭示了"答题能力 ≠ 评分严度"的核心洞察，对语料质检管线的选型有直接影响：最强大的答题模型未必是最合适的质检员。
...
> 数据来源：9 模型深度分析报告 Kruskal-Wallis 检验 + Dunn's post-hoc。完整溯源见附录A #33。
...
但表面数值差异（0.872 vs 0.233）可能来自两种不同机制——是单纯的宽松度差异（大模型更"吝啬"给分），还是存在更深层的判断逻辑分歧？仅靠均值差异无法区分这两种假设。为此设计了四层验证，从排名一致性到领域偏差逐层递进：

**L1 — 尺度差异 vs 逻辑差异**：6×6 上三角矩阵中 Spearman ρ = 0.230 ≈ Pearson r = 0.230，Δ < 0.01。不一致来自逻辑分歧而非单纯尺度差异。Qwen 家族内 ρ ∈ [0.29, 0.60] 同架构尚可，但跨架构配对（涉及 Llama）出现负相关。

![Figure 4.2](shared/assets/judge_spearman_heatmap.png)
*Figure 4.2: 6 个本地 Judge 对 80 题评分的 Spearman 秩相关热力图。数据来源：本地 Judge 反规模定律分析报告。生成脚本：judge_spearman_heatmap.png。*
...
![Figure 4.3](shared/assets/judge_zero_score_rate.png)
*Figure 4.3: 本地 Judge 零分率随模型尺寸的增长趋势。数据来源：本地 Judge 反规模定律分析报告。生成脚本：judge_zero_score_rate.png。*

**L3 — API 共识对齐**：Qwen3.5-4B 与 API 8 Judge 共识对齐度最高（Spearman ρ=0.481），Llama3.2-3B 在本次评测中 Spearman 秩相关系数最低（ρ=0.057）。无本地 Judge 能可靠逼近 API 专家集判断，多 Judge 融合不可替代。

![Figure 4.4](shared/assets/judge_api_consensus_alignment.png)
*Figure 4.4: 各本地 Judge 与 API 8 Judge 共识裁判集的 Spearman 秩相关。数据来源：本地 Judge 反规模定律分析报告。生成脚本：judge_api_consensus_alignment.png。*

**L4 — 领域特化偏差**：所有 Judge 的跨领域方差 < 0.01，评分倾向在学科间稳定，领域偏差不是评分差异的主要来源。

![Figure 4.5](shared/assets/judge_domain_bias_heatmap.png)
*Figure 4.5: 各 Judge 在控制科学子领域间的评分方差热力图。数据来源：本地 Judge 反规模定律分析报告。生成脚本：judge_domain_bias_heatmap.png。*
...
为验证这一假说，对 35B 执行了 **think on/off 对照实验**：从 80 题分层样本中选取 10 题（5B + 5D，跨 L2-L4），使用相同的 Judge prompt 和评分标准，唯一的差异是 think=true（开启显式推理链）vs 基线 think=false。结果显示：开启思考后，35B 对同一批答案的 10 题均值从 **0.200 升至 0.545（Δ=+0.345）**，原本大量被判零分的回答在充分推理后被重新评价为正确或部分正确（如 B/L2 题从 0.0→1.0，D/L4 题从 0.0→1.0）。这一结果提示，35B 在 think=false 模式下的极低评分（整体均值 0.233、零分率 33.8%）至少部分源于推理时间预算的约束，而非模型固有的评分倾向——当给予充分的显式推理预算后，35B 的评分显著向宽松方向回归。因此，"反规模定律"中 35B 的极端严格度确实混杂了推理时间预算的不对等效应，但即使在修正这一混杂后，Qwen 家族内部的严格度单调递增趋势（2B→4B→9B→35B）依然成立——反规模定律在原始观测中高估了 35B 与 gemma 之间的差距，但结论方向不变。
...
| 错误类别 | 频次 | 占比 | 典型表现 |
|:---|:---:|:---:|:---|
| 概念混淆 | 23 | 47.9% | 术语/定义理解偏差——如将 CFH-Hard 误读为"集中式-联邦式-层次式"而非"攻击代理与用户代理高度重叠" |
...
> 数据来源：失败分析报告 48 case 抽样分类。完整溯源见附录A #34。
...
> 数据来源：QLoRA 微调评测报告与基线评测报告的维度分交叉对比。完整溯源见附录A #13-16。
...
> 数据来源：9B QLoRA 微调评测报告 + PPL 双尺寸差值报告。4B row 来自 9B 适配器跨尺寸加载对照实验。完整溯源见附录A #13-16。
...
> 数据来源：9B QLoRA 适配器跨尺寸加载至 4B text-only 基座的 hold-out 89 题评测报告。完整溯源见附录A #24。
...
> 数据来源：Agent 在三个架构族上独立执行的 QLoRA 训练→评测管线，89 题 cross_arch test set，Judge 统一 deepseek-v4-flash。完整溯源见附录A #25-30。

### 4.4 PPL 悖论与公式识别对比

#### 4.4.1 PPL 悖论：语料质量的信息论证明
...
**对自动化语料产线的工程禁令**：PPL 不能用作语料质量的代理指标。在语料生产中，一个常见的工程直觉是"微调后 PPL 降了多少 → 语料质量有多好"——本实验首次以定量证据推翻了这个假设。PPL 降 53.6% 而评分仅退 0.73%，意味着 PPL 变化的主因是格式适配（模型学会了领域文本的统计模式），而非推理能力提升。任何以 PPL 变化作为语料质量验收标准的自动化管线都是在度量噪声。

#### 4.4.2 公式识别对比

如前 §2.5 所述，在同一台 RTX 5090 上完成了 MinerU 1.2B VLM 与 MiMo-V2.5 的 42 条公式对比。在四轨引擎架构中，这一对比验证了轨道3（vLLM）的独特价值：1.2B 专模在字符匹配率上反超 310B 通用模型（0.0216 vs 0.0156），意味着对于公式识别这类格式敏感任务，专用小模型的精细度优于通用大模型。1/258 的参数效率使 vLLM 轨道成为一种低成本高精度的专项选择——当轨道2（Ollama）处理通用推理、轨道1（API）处理视觉审计时，轨道3 专门覆盖了文档解析的精度瓶颈环节。这一发现是在全量解析过程中自然涌现的，而非预先设计——Agent 在编排 multi_format_parse 和 mineru_parse 时，自动将 MinerU 输出的公式图片送达 vLLM 做二次验证，这一自动化质量闭环证明了多轨协同架构的实用价值。
...
以下容错验证的全部场景——从 Provider 降级到 Step 重试到 Task 级 Checkpoint 恢复——均来自系统运行日志中的真实故障记录，非事后构造的演示案例。系统实现三层容错机制，覆盖 Provider、Step、Task 三个级别的故障场景：
...
可复现性保证通过以下措施实现：固定数据集（git 跟踪）、原子写入（不丢不坏）、LLM 参数固定（确定性配置）、跨管道验证（MAE=0.0003）、一键复现脚本。端到端复现验证日志记录了从数据集校验（500 题 0.9 秒四维平衡校验通过）到评测执行到排行榜生成的完整可复现链路。
...
| 部署文档 | 708 行（`track2_agent_deploy.md`） | 文件行数 |

### 5.6 性能剖面

四轨引擎在同一台 RTX 5090（24GB）上的实测推理延迟构成完整的性能剖面：

**本地推理 Elastic Scaling**：同一道 C 维控制题在三代尺寸上的延迟呈近线性增长——4B（4GB 显存）46.3s、9B（5.5GB）54.7s、35B（23GB）128.2s。最大模型单题推理仍在 2 分钟内完成，无需云计算即可处理全尺寸评测。

| 轨道 | 引擎 | 典型延迟 | 吞吐（qwen3.5 系列） | 部署位置 |
|:---|:---|:---:|:---:|:---|
| 轨道1 | DeepSeek v4-flash / MiMo-V2.5 | 1-5s/次（远端 API） | — | 远端 |
| 轨道2 | Ollama 本地推理 | 0.5-1.5s/题（文本推理） | 35B: ~5.9s/q（控制科学题），9B: 96.8 tok/s | 本地宿主机 |
| 轨道3 | vLLM MinerU-1.2B VLM | 0.3-1s/张（公式图片） | 112 条/秒（嵌入） | WSL2 容器 |
| 轨道4 | Ollama Vision (qwen3.5:9b) | ~16.6s/张（730 张医学图实测） | 描述均值 818 字符 | 本地宿主机 |

**全链路时间分布**（D 数据飞轮 391s 的 6-step 拆解）：
...
飞轮端到端 391s 中 GPU 本地推理（corpus_parse 180s）占 46%，API 调用（benchmark_build + quality_arbitrate + model_evaluate 合计 ~160s）占 41%，本地 CPU + I/O 占 13%。这一分布说明了四轨架构的设计优势——API 密集型任务和 GPU 计算任务天然分离，可独立优化而无资源争用。

> 数据来源：Agent 执行日志（demo-data-flywheel.json）+ Elastic Scaling 实测（demo-35b-elastic-proof.json）+ Ollama 吞吐基准。完整溯源见附录A #31。
...
Agent 框架的跨领域迁移能力在医学文献处理中得到了验证。从控制科学领域迁移到医学文献场景时，Agent 的 13 个 intent 中有 17 个核心模块未做任何修改：
...
这一迁移效率的核心原因不是"Agent 做得特别通用"，而是**控制科学和医学共享同一数学工具谱系**——两者都使用微分方程、系统辨识、最优控制等数学工具。在医学文献的领域迁移实验中，100 篇 PMC 文献的端到端处理在 Agent 框架下约半日内完成，其中 72 篇核心文献天然使用控制方法（13 个"控制×医学"交叉方向），证明了控制科学作为 AGI4S 基座数学语言的渗透性。同一 Agent 框架已扩展支持图像+RAG 联合推理——轨道 4（Ollama Vision）在 MedBench VLM 10 子集 300 题上完成全量验证，零错误（详见赛道三 §5.6）。

### 6.2 双赛道联动：同一资产的两种角色

赛道一（Sci-Align 评测基准）与赛道二（Data Agent）在同一仓库内构成互补生态——同一批核心资产在两个赛道扮演不同角色，产生跨赛道的统一洞察：
...
**核心叙事**：Agent 同时驱动两个赛道。赛道一产出的 362 篇语料库、28K chunk、500 题评测数据集、9 模型排行榜和 QLoRA 微调结果，全部作为赛道二 Agent 的语料处理与评测能力展示素材。Agent 的 13 intent 中 intent 5~9（benchmark_build / quality_arbitrate / model_evaluate / multi_judge_compare / leaderboard_viz）直接对应赛道一的评测管线，使赛道一评测流程可一键复现。
...
这一联动范式为"一份数据投两赛道"提供了实践参考：评测基准为 Agent 提供测试场，Agent 为评测基准提供自动化引擎，两者构成的闭环本身就是一场完整的科学发现演示。

### 6.3 方法论通用性

Agent 方法论的通用性体现在三个层面：

1. **任务层面**：13 Intent 覆盖的"检索→解析→结构化→质量审计→评测→报告"链是通用范式，不限于控制科学。在医学文献场景中，intent 序列完全复用，仅数据源从 arXiv 换为 PMC。
...
AI 驱动最大化设定了一个精确的边界条件：**目标层由人工定义，约束层由人工与 AI 共同固化，执行层由 AI Agent 最大化承担**。具体来说：

- **目标层（人类负责）**：四维评测体系（A/B/C/D 各 125 题的设计原则）、13 个 Agent Intent 的能力边界、医学章节本体与任务目标
- **约束层（人类定义协议，AI 执行校验）**：LLM-as-Judge 的评分标准模板、数据 schema、失败判定规则、日志字段、合规边界
- **执行层（AI Agent 最大化负责）**：文献检索的关键词选择与下载、格式自动检测与解析引擎选择、出题数量与难度分配、仲裁过滤、评测 provider 路由、排行榜更新、异常重试与 checkpoint 恢复

这一边界的合理性在于：设计是"做什么"和"什么算合格"的决策，需要人的领域理解与风险判断；执行是"在既定协议内怎么完成"的决策，可以被工具调用、日志、checkpoint 和自动评测约束。将设计、约束、执行三层拆开，避免了"无人类参与"的不可辩护说法，同时保留了本项目的核心主张：凡是可以被协议化、复现化、审计化的环节，都应尽可能交给 AI Agent。
...
| 无人类控制科学家基线 | 9 个模型在 Benchmark 上的得分极差 0.21，这一区分度本身就证明了评测体系的有效性。在 AI 评测中，人类基线的价值常被高估——能力可测量不等同于价值可衡量 |
| 医学仅 100 篇 / 25 查询 | 25 查询 × 5 候选 × 14 Judge = 1,050 条评分。统计效力来自 Judge 矩阵维度，而非查询数量。100 篇是精心筛选（curate），非随机抽样（sample） |
| 单张 RTX 5090 | 刻意在消费级硬件上完成全部实验，证明垂直领域 AI 工作流不依赖云计算。对数据隐私敏感场景（医院、军工）有直接落地价值 |
...
```powershell
.\run_reviewer_demo.ps1 -Track 2   # Agent 能力注册 + dry-run 计划（~2s, 零API）
...
4. **跨领域迁移可行性**：Agent 的 17 个核心模块从控制科学零代码迁移到医学文献场景，追加代码仅约 30 行。跨领域 PPL 一致性（控制 -53.6% vs 医学 -47.0%）在两个独立领域中得到了一致复现，提示该现象可能在垂直领域 QLoRA 中具有跨领域稳定性。

### 7.2 核心价值

Agent 的核心价值不在于"有一个 13 Intent 的系统"，而在于**人类定义目标与约束后，从解析到评测的可日志化执行环节由 AI Agent 最大化承担**。D 数据飞轮 391 秒的 6-step 闭环——Agent 在每个环节按协议选工具、记录决策、自动恢复——是对这一价值的最小可验证单元。跨领域迁移（§6.1）进一步证明这一价值不限于单一领域。

两个场景的验证——控制科学语料自动化处理与医学文献跨领域迁移——表明垂直领域 AI 工作流的核心壁垒不在模型参数量，而在结构化数据的工程链路。当这一链路被自主化后，模型可以很轻量（4B、9B），部署可以很简单（一台 RTX 5090），迁移可以很高效（零代码改动）。

### 7.3 数据与代码共享

项目核心资产已在 GitHub 开源（MorningStar0709/ControlSci，Apache 2.0 许可），仓库包含 Benchmark 评测框架、Agent 编排系统、QLoRA 训练脚本和部署指南四个子模块。Hugging Face 数据集 `MorningStar0709/control-sci-corpus`（CC-BY-4.0 许可）包含四个子集：
...
数据集可通过 `datasets.load_dataset("MorningStar0709/control-sci-corpus")` 零适配加载——无需 ETL、无需索引构建、无需依赖注入。CC-BY-4.0 许可确保学术引用和商业使用均无障碍。

三个自研 Trae Skill 在社区发布，各自解决了此前必须人工介入的关键环节：
...
| **writing-retrospective-doc** | 工程回顾文档自动化模板生成 | 事后手动整理实验日志和工程纪实 |

mineru-to-md Skill 经 362 篇科学文档全量解析验证，经历了五个生产故障驱动的能力进化（DETACHED_PROCESS 长任务隔离 / GPU 显存生命周期感知 / stats 统计注入审计 / 双模式耗时模型 / skip-existing 断点续跑），已作为生产级工具向社区开放。三个 Skill 共同构成了"文献检索→文档解析→工程回顾"的 AI 驱动工具链——从论文被检索到文档被解析、再到工程经验被沉淀，重复执行环节不再依赖人工逐步操作。

项目在 Trae IDE 社区获得 **Community Star（活力之星）** 认证，三个自研 Skill 均在社区公开发布并持续维护。MinerU to MD Skill 在社区中解决了 Windows 环境下 Docker + WSL2 桥接的 GPU 生命周期管理这一跨平台集成痛点——此前无同类工具覆盖此场景。同一套 mineru-to-md 引擎在赛道三中零代码迁移至医学文献场景（98 篇 PMC），验证了其跨领域的通用性。

![Figure 7.1](shared/assets/community_star.jpg)
*Figure 7.1: Trae IDE Community Star 社区认证。数据来源：Trae IDE 社区平台。生成方法：平台截图。*

---

## 评委验证路径

本报告中的 Agent 能力声明均可通过以下路径独立验证。建议评委优先阅读 [judge_quickstart.md](judge_quickstart.md) 获取三赛道统一导航，再通过 Agent 20 题样例包逐项抽查。

| 步骤 | 操作 | 对应文件 / 命令 | 赛道二评分映射 | 耗时 |
|:----|:-----|:--------------|:------------:|:---:|
| 1 | 三赛道统一导航 | [judge_quickstart.md](judge_quickstart.md) → §4 "赛道二验证路径" | 全赛道 5 维评分细则对照 | 3 min |
| 2 | 20 题抽样验证（含失败恢复、降级案例）| [track2_agent_20_cases.md](track2_agent_20_cases.md)，逐题对照 Intent / Agent Plan / Tool calls / Output / Recovery flow | 每题标注 Related score item | 5 min |
| 3 | 一键复现核心指标 | `.\run_reviewer_demo.ps1 -Track 2` | 赛道二 8 维验收测试 | 2 s |

**API + 5090 混合架构原则**：Agent 的四轨推理引擎（API DeepSeek/MiMo → Ollama 本地 qwen3.5:9b/35b → vLLM MinerU 1.2B VLM → Ollama Vision qwen3.5:9b）在单张 RTX 5090 上全栈部署。API 负责需要最强推理能力且输入为公开/脱敏材料的环节（Intent 路由、Judge 仲裁、误差分析）；本地模型承载文本推理、嵌入检索、公式识别、医学图片描述、原文解析、chunk 索引与微调数据处理等高频或隐私敏感任务。两条轨道共享同一 agent_cli.py 编排框架和 LogStep 审计 schema，通过 ResourceScheduler 自动选择 provider，并以 `data_policy` 固化"公开/脱敏任务上云，原文/医疗/微调/中间 chunk 留本地"的边界。
...
| 17 | 跨管道 MAE=0.0003 | cross_pipeline_reproducibility.md | — |
| 18 | 双重身份 Pearson r=0.999991 | dual_role_matrix.png 数据 | — |
| 19 | 飞轮耗时 391s | demo-data-flywheel.json meta.duration_ms | Agent 日志 |
| 20 | 跨模态对齐 79.3%（一致率 75.9%）| cross_modal_audit_summary.json | `cross_modal_audit_summary.json` |
| 21 | MinerU 1.2B vs MiMo 公式对比 | mineru_1.2b_vlm_formula_comparison.md | — |
| 22 | 本地 Judge 6 模型评分 | scoring_anti_scaling_law.md | — |
| 23 | 灾难性遗忘 9B 0.6249→0.011 零分率 99% | qlora-9b-training-demo.md | — |
| 24 | LoRA 跨尺寸移植 4B 0.467→0.464 | qlora_cross_size_c_dimension.md | — |
...
| 29 | 三 Judge Fleiss' κ=0.575（P̄=0.827, P̄ₑ=0.593）| cross_judge_kappa.json | 30 题三 Judge 交叉验证 |
| 30 | 质量审计 Overall=4.17/5.0（清晰度 4.18 / 正确性 4.30 / 难度对齐 3.60 / 维度匹配 4.62）| quality_audit.json | 50 题抽样审计 |
| 31 | 35B elastic scaling 延迟：4B 46.3s / 9B 54.7s / 35B 128.2s | demo-35b-elastic-proof.json | Agent 执行日志 |
| 32 | 自我修正实验：20 题 50% 成功率，均分 0.10→0.452（A 维 80%），3 例 Δ=+1.00 | self_correction_report.md | Agent Verifier 闭环修正 |
| 33 | 9 模型 Kruskal-Wallis H=89.95（p<0.001），Dunn's post-hoc 13/36 显著 | t2_deep_analysis.md | 统计显著性检验 |
| 34 | 失败分析 48 case：概念混淆 23 / 推理断裂 10 / 设计发散 10 / 条件盲区 5 | failure_analysis.md | 零分根因分类 |
| 35 | think on/off 对照实验：35B 10 题 B/D 维 Δ=+0.345（think=false 0.200→think=true 0.545）| `benchmark/eval/results/think_probe_35b_10queries.json` | `python benchmark/eval/think_probe.py` |
...
**安全策略**：所有 API Key 仅通过 `os.getenv()` 读取，零硬编码。`git grep "sk-" -- *.py *.json *.md` 零命中。
...
| 日志 | 描述 | 报告引用 |
|:----|:----|:--------|
| `demo-data-flywheel.json` | D 数据飞轮自动执行实录（391s 6-step） | §3.5（案例1） |
| `demo-local-gpu-fallback-20260509-0116.md` | Ollama 未启动 → script 降级（62ms） | §3.5（案例5） |
| `demo-35b-elastic-proof.json` | 4B→9B→35B 弹性伸缩证明（46→55→128s） | §5.6 |
...
| `_verify_10_intents.py` 验收日志 | 13 intent × 8 维度验收测试全通过 | §3.1 |
| `visual_audit_results.jsonl` | 跨模态全量审计结果（9,227 条判决） | §2.3 / §3.5（案例2） |
| `qlora-9b-training-demo.md` | 9B QLoRA 微调 + 灾难性遗忘实验全纪录 | §4.3.2 |
| `scoring_anti_scaling_law.md` | 本地 Judge 反规模定律 L1-L4 全量分析 | §4.2.2 |
| `cross_pipeline_reproducibility.md` | Leaderboard ↔ Consolidated 双管道评分对比 | §4.1 |

日志内容包含赛道二提交要求中的所有关键字段：任务输入、执行步骤、调用工具信息、最终输出结果。所有日志路径以项目根目录 `benchmark/agent/logs/` 为基准。

---

> **文档状态**：v3.1（润色版） | **数据截止**：2026-05-12
> **Agent 执行案例**：§3.5 含 6 个典型任务案例（D 飞轮 / 跨模态审计 / QLoRA 闭环 / 多格式批处理 / 失败恢复 / 开放指令拆解）
> **AI 驱动最大化边界界定**：§6.4 精确说明目标层、约束层与执行层的分工
> **跨领域迁移**：§6.1 详述 17 个核心模块零代码改动 + 追加 ~30 行
> **评分维度对照**：§1.4 含赛道二 5 维评分细则→章节映射表（含满分分值与关键证据索引）
> **运行日志索引**：附录 C 含 10 条关键运行日志与对应章节链接
> **数据验证声明**：本报告中所有量化数字同一份 `shared/DATA-TRACE.md` 交叉验证。附录 A 中 35 条条目可直接溯源至权威源文件。
