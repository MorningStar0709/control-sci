...
| 12 | 失败恢复 | `benchmark_build` | Step 级 Self-Improve | `agent_failure_recovery_cases.md` 案例 2 |
| 13 | 失败恢复 | `mineru_parse` | File 级跳过脏文件 | `agent_failure_recovery_cases.md` 案例 3 |
| 14 | 失败恢复 | `cross_modal_audit` | API 超时重试 | `agent_failure_recovery_cases.md` + 代码 |
...
| 17 | 降级处理 | `corpus_parse` | 动态重规划跳过下游 | `agent_failure_recovery_cases.md` |
| 18 | 审计日志 | 全链路 | 结构化日志 schema | `log_schema.py` + `logs/demo-10intents-e2e-20260509.md` |
| 19 | 审计日志 | 跨管道验证 | 双管道可复现性 | `cross_pipeline_reproducibility.md` |
...
| **Related score item** | D3 — 精准理解复杂指令；Agent 自主完成论文检索 |
...
| **Related score item** | D1 — 文档解析与结构化处理能力；D3 — 合理调用 MinerU 工具 |
...
| **Related score item** | D1 — 信息抽取准确性、数据清洗质量；D3 — 目标拆解为多步子任务 |
...
| **Related score item** | D3 — 合理调用工具；目标拆解为可执行子任务 |
...
| **Related score item** | D3 — 多步骤逻辑性、合理调用工具；D4 — 输出可靠性 |
...
| **Related score item** | D3 — 合理调用工具；D4 — 可复现性保证（统一评测管线） |
...
| **Related score item** | D3 — 全链路闭环；D4 — 输出可靠性、结构化日志 |
...
| **Related score item** | D1 — 格式转换规范性；D3 — 合理调用 MinerU 工具 |
...
| **Related score item** | D3 — 合理调用工具（MiMo-V2.5 原生视觉）、per-intent 参数精确传递；D2 — 跨模态对齐审计 |
...
| **Related score item** | D3 — 三轨推理引擎、成本速度精度平衡；D4 — 弹性伸缩 |
...
| **Related score item** | D4 — Provider 级容错、自动降级机制 |
...
| **Related score item** | D3 — 自改进闭环、异常恢复；D4 — 步骤级容错 |
| **Boundary note** | 当前 Agent 执行日志中所有 intent 首次尝试 score=1.0，该机制处于待触发状态——作为安全网存在；最多 2 次重试均失败则标记 failed |
...
| **Related score item** | D4 — 文件级容错；D1 — 数据清洗质量 |
...
| **Related score item** | D4 — 运行稳定性、故障恢复矩阵 |
...
| **Related score item** | D4 — 隐私双路径对称切换、运行稳定性；D3 — 成本速度精度平衡 |
...
| **Related score item** | D4 — 运行稳定性、任务级容错 |
...
| **Log path** | `agent_failure_recovery_cases.md` — 动态重规划案例（§3.7 完整描述） |
| **Pass criteria** | Agent 正确检测上游产出不足；下游步骤被跳过而非报错；执行日志含跳过原因说明 |
| **Related score item** | D3 — 动态重规划、Agent 自主调整执行计划 |
...
| **Related score item** | D4 — 可追溯日志、输出可靠性 |
...
| **Log path** | `benchmark/eval/results/analysis/cross_pipeline_reproducibility.md`（完整分析）；`DATA-TRACE.md §八 #63`（可复现命令） |
| **Pass criteria** | 两条管道评分偏差 < 0.001（MAE/RMSE）；排名完全一致 (ρ=1.0)；可复现命令清晰记录 |
| **Related score item** | D4 — 可复现性保证体系、跨管道验证 |
...
| **Related score item** | D4 — 任务级容错、Checkpoint 恢复机制；D1 — 跨模态对齐审计 |
...
| **Related score item** | D3 — Agent 将复杂意图自主拆解为 7 步可执行 Plan，跨 3 类 7 种工具自主编排；D4 — 全链路可复现，同一条命令可重复产生一致结果 |
...
| **Related score item** | D1 — MinerU 本地 vLLM 部署能力（5090）+ 双引擎公式识别对比；D2 — 跨模态对齐审计的双引擎交叉验证方法 |
...
| **Related score item** | D1 — 多格式归一化解析能力（4 格式 × 统一 schema）；D2 — 跨模态对齐审计（图文一致性判决）；D3 — 跨工具管线编排，per-intent 参数精确传递 |
...
| **Related score item** | D4 — 任务级 Checkpoint 容错，中断恢复不断数据；D3 — Agent 自主管理长时运行任务的执行状态 |
...
| **Related score item** | D2 — 跨模态对齐审计全量证据链（4,996 chunk 全覆盖）；D4 — Checkpoint 原子落盘保障、--resume 幂等恢复；结构化格式可 grep/jq 按 judgment 类型筛选 |
...
| `docs/evidence/agent_failure_recovery_cases.md` | 三层容错架构详细案例文档 |
| `benchmark/eval/results/analysis/cross_pipeline_reproducibility.md` | 跨管道可复现性验证 |
